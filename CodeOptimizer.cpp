...
namespace BT{
	...
	CodeOptimizer::CodeOptimizer(std::list<unsigned int>& _entrypoints, std::vector<bt_instruction> &instructions, coLevel _level)
	...
	/**/
	bool CodeOptimizer::OptimizeToZeroLoop(CodeTapeIterator &it, const RepairFn2 & repairCB)
	{
		CodeTapeIterator n;
		int lim, s;
	...
	void CodeOptimizer::RelinkCommands(const CodeTapeIterator& start, short n)
	{
		RelinkCommands(start, instructions.end(), n);
	}

	void CodeOptimizer::RelinkCommands(const CodeTapeIterator& start, const CodeTapeIterator& end, short n)
	{
