...
namespace BT {
	...
	void CodeAnalyser::Repair(void)
	{
		...
		while (it < parser.instructions.end())
		{
			...
			if (TestForInfiniteLoops(it,
				...
				_it = parser.instructions.erase(_it);
				RelinkCommands(_it, parser.instructions.begin() + (_it->jump), 1); //�wiadomie uzywam starej pozycji
				RelinkCommands(parser.instructions.begin() + (_it->jump) + 1, 2); //od ko�ca usuni�tej p�tli juz po dwa
				++repaired_issues;
			}))
				continue;
			...
			if (TestForFunctionsErrors(it,
				...
				parser.instructions.erase(parser.instructions.begin() + (_it->jump));
				_it = parser.instructions.erase(_it);
				this->RelinkCommands(_it, parser.instructions.begin() + (_it->jump), 1); //�wiadomie uzywam starej pozycji
				this->RelinkCommands(parser.instructions.begin() + (_it->jump) + 1, 2); //od ko�ca usuni�tej p�tli juz po dwa
				++repaired_issues;
			}))
				continue;
	...
	bool CodeAnalyser::TestForFunctionsErrors(CodeTapeIterator& it, const RepairFn2& emptyFunType1Rep, const RepairFn& emptyFunType2Rep)
	{
		...
		{
			n = parser.instructions.begin() + it->jump;//bierzemy ca�� tre�� funkcji
	...
	bool CodeAnalyser::TestForRepetition(CodeTapeIterator& it, const RepairFn2& repairCB)
	{
		...
		if (it + 1 < parser.instructions.end())
		{
			...
			if (n != parser.instructions.end()) //jest jakies powt�rzenie
			{
